...
function countVowels(str) {
  ...
  return str.split('').filter((n) => vowels.includes(n)).length;
}
...
/**
 * Find the longest word in the sentence. If there are multiple longest words,E
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.split(' ').length; i += 1) {
    if (sentence.split(' ')[i].length > word.length) {
...
function reverseWords(str) {
  ...
  for (let i = 0; i < splitStr.length; i += 1) {
    reverseStr.push(splitStr[i].split('').reverse().join(''));
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (upperStr[i] === str[i]) {
      invertList.push(str[i].toLowerCase());
    } else {
      invertList.push(str[i].toUpperCase());
    }
  }
  return invertList.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  for (let i = 1; i < splitValue.length; i += 1) {
    if (splitValue[i][0] === splitValue[i][0].toUpperCase()) {
      listName.push(splitValue[i]);
    }
  }
...
function encodeToRot13(str) {
  const newStr = [];
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].charCodeAt() < 91 && str[i].charCodeAt() > 64) {
      if (str[i].charCodeAt() > 77) {
        newStr.push(String.fromCharCode(str[i].charCodeAt() - 13));
      } else {
        newStr.push(String.fromCharCode(str[i].charCodeAt() + 13));
      }
    } else if (str[i].charCodeAt() > 109 && str[i].charCodeAt() < 123) {
      newStr.push(String.fromCharCode(str[i].charCodeAt() - 13));
    } else if (str[i].charCodeAt() > 95 && str[i].charCodeAt() < 110) {
      newStr.push(String.fromCharCode(str[i].charCodeAt() + 13));
    } else {
      newStr.push(str[i]);
    }
